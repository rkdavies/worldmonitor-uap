...
<details>
<summary><strong>UAP Situational Awareness</strong> (UAP variant)</summary>

The UAP (Unidentified Anomalous Phenomena) variant provides a dedicated dashboard for global UAP monitoring, aligned with institutional and citizen-science data sources.

- **UAP News** — The Debrief, Liberation Times, Space.com, and curated Google News UAP/AARO/NASA feeds
- **Institutional** — NASA UAP, AARO/DoD, and Pentagon UAP–related coverage
- **Investigative** — The Black Vault, Weaponized podcast, and UAP disclosure/legislative coverage
- **Scientific** — Galileo Project and scientific UAP research feeds
- **Sightings** — NUFORC, Enigma Labs, and sighting-report feeds
- **Anomalous Activity Index (AAI)** — 90-day regional sighting density; **adjusted AAI** down-weights regions with high *reporting opportunity* (literature-aligned proxies after Medina et al., *Scientific Reports* 2023: sky visibility, airport/air traffic, military-training context). **Raw score** remains unadjusted density. **Residual index** is peer-normalized after expectation weighting. This frames *reporting conditions*, not proven anomalous physics.
- **Sensor stations** — Static layer for Galileo Project (Harvard) and Skinwalker Ranch
- **Map layers** — UAP sightings, sensor stations, historical hotspots, **reporting context** (one national context index per country; **large countries** show **multiple disks** via optional `mapAnchors` in `shared/uap-observation-context-by-region.json`—same index everywhere, sub-labels e.g. USA West / Northeast); smaller regions use a single anchor; **military**, **bases**, **flights**, **nuclear**, **conflicts**, **weather**

**Observation context** — `shared/uap-observation-context-by-region.json`: **`airTraffic`** is regenerated from [OurAirports](https://ourairports.com/data/) (CC0): counts of `large_airport` + `medium_airport` per country, log-scaled 8–95 among tracked regions. Run `npm run ingest:uap-observation-context` (network) to refresh; provenance in `shared/uap-observation-ingest-meta.json`. **`sky`** and **`militaryTraining`** remain expert-seeded until VIIRS / FAA SUA ingestion lands.

**API**: `/api/uap/v1` — `list-uap-sightings`, `list-uap-institutional-reports`, `list-uap-sensor-stations`, `get-aai-scores` (returns `observationContextIndex`, `adjustedAaiScore`, `residualActivityIndex`, `methodologyNote`). Data can come from data.world (if `DATA_WORLD_TOKEN` is set), live NUFORC scrape, or Redis seeded from the [local nuforc_sightings_data repo](/uap-nuforc-repo) (last 200, updated regularly).

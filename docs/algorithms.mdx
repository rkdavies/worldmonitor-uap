...
### Anomalous Activity Index (AAI) — UAP variant

Regional **raw score** (0–100): 90-day NUFORC (or data.world) sighting count normalized to the busiest region (up to 70 points) plus optional institutional activity.

**Observation context index** (0–100): Weighted blend of proxies for sky visibility, commercial/military air traffic context, and military-training context per region (`shared/uap-observation-context-by-region.json`), with relative weights aligned to Medina et al., *Scientific Reports* (2023). The **airTraffic** field is refreshed from OurAirports (large + medium airports per country, log-scaled among tracked regions) via `npm run ingest:uap-observation-context`; see `shared/uap-observation-ingest-meta.json` for counts and timestamp.

**Expectation multiplier** \(E\): Maps context index to ~0.42–1.78. **Effective count** = sightings\(_{90d}\) / \(E\). **Adjusted density** = min(70, effective / max(effective)×70). **Adjusted AAI** = raw score plus the delta between adjusted and raw, **capped at ±15 points** (prevents single-factor domination). **Residual index** (0–100) = effective / max(effective)×100 across returned regions. **Level** bands apply to adjusted score. This does **not** assert more physical UAP activity—only activity relative to modeled reporting opportunity.
